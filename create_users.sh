...
do
	...
	echo "INSERT INTO impi (id_imsu, identity, k, auth_scheme, default_auth_scheme, amf, op, opc, sqn, ip, line_identifier, zh_uicc_type, zh_key_life_time, zh_default_auth_scheme) VALUES (@imsu_id, 'contract$i', '$i-$PASSWORD-$i', 255, 4, '', '', '', '000000000000', '', '', 0, 3600, 1);" >> ./create.sql
	echo "set @impi1_id=last_insert_id();" >> ./create.sql
	echo "INSERT INTO impi (id_imsu, identity, k, auth_scheme, default_auth_scheme, amf, op, opc, sqn, ip, line_identifier, zh_uicc_type, zh_key_life_time, zh_default_auth_scheme) VALUES (@imsu_id, '$PREFIX$i@$DOMAIN', '$i-password-$i', 255, 4, '', '', '', '000000000000', '', '', 0, 3600, 1);" >> ./create.sql
	...
	echo "INSERT INTO impu_visited_network (id_impu, id_visited_network) SELECT @impu2_id, id FROM visited_network;" >> ./create.sql
